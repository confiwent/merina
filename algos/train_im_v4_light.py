...
UPDATE_PER_EPOCH = 20 # update the parameters 8 times per epoch
RAND_RANGE = 1000
ENTROPY_EPS = 1e-6
...
def train_iml_v4(train_epoch, net_env, valid_env, args, add_str, summary_dir):
    ...
    with open(log_file_name + '_record', 'w') as log_file, \
        ...
        if args.gru:
            ...
            optimiser_vae = torch.optim.RMSprop(
                vae_net.parameters(), 
            ...
            optimiser_vae = torch.optim.Adam(
                vae_net.parameters(), 
                lr= LEARNING_RATE_VAE
            )
        ...
        for _ in range(train_epoch):
            ...
            model_actor.train()
            vae_net.train()
            if memory.return_size() >= 2 * minibatch_size:                
                ...
                for _ in range(UPDATE_PER_EPOCH):
                    ...
                    # pdb.set_trace()
                    probs_ = model_actor.forward(batch_states, \
                                            batch_latents).clamp(1e-4, 1.0 - 1e-4)
                    ...
                    for idx in range(sample_num):
                        probs_ = model_actor(batch_states, latent_samples[idx]).clamp(1e-4, 1.0 - 1e-4)
            ...
            if epoch % args.valid_i == 0 and epoch > 0 :
                ...
                model_vae = vae_net
                mean_value = valid(args, valid_env, model_actor, model_vae, epoch, \
                ...
                # save models
                save_models(
                            logging, summary_dir, add_str, 
                            model_actor, vae_net, epoch, 
                            max_QoE, mean_value
                )
