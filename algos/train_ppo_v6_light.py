...
from .AC_net_v6_light import Actor, Critic
        ...
        while True:
            ...
            while memory.return_size() < args.explo_num * args.ro_len:
                ...
                for _ in range(steps_in_episode):
                    ...
                    r_ = float(max(reward, -2*reward_max) / reward_max)
            ...
            for _ in range(ppo_ups):
                ...
                loss_actor = 1 * lc_alpha * loss_clip_actor - \
            ...
            if epoch % int(args.valid_i /2) == 0 and epoch > 0:
                mean_value = valid(args, valid_env, model_actor, vae_net, epoch, \
